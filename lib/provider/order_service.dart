...
class OrderService with ChangeNotifier {
  ...
  Future getOrder(context, String? startDate, String? endDate,String? dayOption, String? orderState, String? rpaState, String? staffName,int? page, String? searchColumn, String? searchValue ) async {
    Logger logger = Logger();
    UserModel? user = await App().getUserInfo();
    orderList.value = List.empty(growable: true);
    Map<String,dynamic> api24Data  = Map<String,dynamic>();
    int totalPage = 1;
    await DioService.dioClient(header: true).getOrder(user.authorization, startDate, endDate, dayOption, orderState, rpaState,staffName, page, searchColumn, searchValue ).then((it) async {
      ReturnMap _response = DioService.dioResponse(it);
