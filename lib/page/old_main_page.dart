...
import 'package:logislink_tms_flutter/page/subpage/old_appbar_mypage.dart';
import 'package:logislink_tms_flutter/page/subpage/appbar_notice_page.dart';
import 'package:logislink_tms_flutter/page/subpage/old_appbar_setting_page.dart';
...
class OldMainPage extends StatefulWidget {
  ...
  const OldMainPage({Key? key, this.allocId}):super(key:key);
  @override
  _OldMainPageState createState() => _OldMainPageState();
}

class _OldMainPageState extends State<OldMainPage> with CommonMainWidget,WidgetsBindingObserver {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      if(widget.allocId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (context) => OldOrderDetailPage(allocId: widget.allocId)));
      }
  ...
  void handleDeepLink() async {
    ...
    FirebaseDynamicLinks.instance.getInitialLink().then(
          ...
          if (dynamicLinkData != null) {
            ...
            switch(code) {
              case "tmsOrder":
                Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => OldOrderDetailPage(allocId: allocId)));
                break;
            }
          }else{
            return;
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: (){
                Navigator.of(context).push(MaterialPageRoute(
                    builder: (BuildContext context) => OldAppBarMyPage()));
              },
            ),
            ListTile(
              contentPadding: EdgeInsets.symmetric(vertical: CustomStyle.getHeight(10),horizontal: CustomStyle.getWidth(5)),
  ...
  Future<void> goToOrderDetail(OrderModel item,int index) async {
    ...
    Map<String,dynamic> results = await Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => OldOrderDetailPage(order_vo: item)));
  ...
  Future goToRegOrder() async {
    Map<String,dynamic> results = await Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => OldRegistOrderPage(flag: "R")));
  ...
  Future<void> goToTransInfo(OrderModel data) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => OldOrderTransInfoPage(order_vo: data)));
  }
